...
$function  = $app->getInput()->get('function', 'jSelectProducts', 'cmd');
			...
			<table class="table table-sm">
				...
				<tbody>
					<?php foreach ($this->items as $i => $item) : ?>
						...
						<?php
							$Productlink     = 'index.php?Productid=' . $item->id;
							$itemHtml = '<a href="' . $Productlink . '">' . $item->title . '</a>';
							$attribs  = 'data-content-select data-content-type="com_menus.item"'
								. 'data-function="' . $this->escape($function) . '"'
								. ' data-id="' . $item->id . '"'
								. ' data-title="' . $this->escape($item->title) . '"'
								. ' data-uri="' . $this->escape($Productlink) . '"'
								. ' data-html="' . $this->escape($itemHtml) . '"';
						?>	
				
						<tr class="row<?php echo $i % 2; ?>">
							<th scope="row">
								<a class="select-link" href="<?php echo $Productlink ?>" <?php echo $attribs; ?> >
