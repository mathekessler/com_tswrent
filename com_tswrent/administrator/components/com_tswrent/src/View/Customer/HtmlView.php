...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null): void
	{
        ...
        $this->canDo = ContentHelper::getActions('com_tswrent', 'customer', $this->item->id);
	...
	protected function addToolbar()
	{
		...
		ToolbarHelper::title(
			$isNew ? Text::_('COM_TSWRENT_MANAGER_CUSTOMER_NEW') : Text::_('COM_TSWRENT_MANAGER_CUSTOMER_EDIT'),
			'bookmark tswrent-customers'
		);
        ...
        $saveGroup->configure(
            function (Toolbar $childBar) use ($checkedOut, $canDo, $isNew) {
                ...
                if (!$isNew && $canDo->get('core.create')) {
                    $childBar->save2copy('customer.save2copy');
                }
            }
        );
        ...
        $toolbar->divider();
        $toolbar->help('Banners:_New_or_Edit_Customer');
	...
	protected function addEditToolbar(): void
	{
        ...
        ToolbarHelper::title(
			Text::_('COM_TSWRENT_MANAGER_CUSTOMER_EDIT'),
			'bookmark tswrent-customers'
		);

        // If not checked out, can save the item.
        if (!$checkedOut && $canDo->get('core.edit')){
            $toolbar->edit('customer.edit');
            $toolbar->cancel('customer.cancel', 'JTOOLBAR_CANCEL');
        }
